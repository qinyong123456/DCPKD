...
class PromptKD(TrainerX):
    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            # [SPLE] 删除权重文件中，所有带 image encoder 与 text encoder 的参数，发现推理结果无变化。
            for del_name, param in self.model.named_parameters():
                if not param.requires_grad:
                    del state_dict[del_name]
                '''
                elif "compound_prompt_projections" in del_name:
                    del state_dict[del_name]
                elif "compound_prompts_text" in del_name:
                    del state_dict[del_name]
                elif "prompt_learner.proj" in del_name:
                    del state_dict[del_name]
                '''
